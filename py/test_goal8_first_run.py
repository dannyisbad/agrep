...
class Goal8FirstRunIntegrationTests(unittest.TestCase):
    def _fixture(self, root: Path, label: str) -> tuple[Path, Path, str]:
        home = root / "home"
        data = root / "data"
        project = home / ".claude" / "projects" / "goal8-cold-proof"
        project.mkdir(parents=True)
        term = f"goal8coldproof{label}x7q9"
        rows = [
            {
                "type": "user",
                "userType": "external",
                ...
                "cwd": str(root / "work" / "goal8-cold-proof"),
    ...
    def test_cold_machine_surfaces_preserve_their_entire_stdout_grammar(self) -> None:
        ...
        for index, (grammar, argv) in enumerate(cases):
            with self.subTest(grammar=grammar), tempfile.TemporaryDirectory(
                prefix=f"agrep-goal8-first-run-{grammar}-"
            ) as raw:
    ...
    def test_explicit_index_requires_a_real_fts_publication(self) -> None:
        with tempfile.TemporaryDirectory(prefix="agrep-goal8-fts-ok-") as raw:
    ...
    def test_simultaneous_first_searches_never_fail_silently(self) -> None:
        with tempfile.TemporaryDirectory(prefix="agrep-goal8-first-race-") as raw:
    ...
    def test_interrupted_cold_search_allows_an_immediate_retry(self) -> None:
        with tempfile.TemporaryDirectory(prefix="agrep-goal8-first-interrupt-") as raw:
    ...
    def test_explicit_index_is_nonzero_when_real_fts_publication_fails(self) -> None:
        with tempfile.TemporaryDirectory(prefix="agrep-goal8-fts-fail-") as raw:

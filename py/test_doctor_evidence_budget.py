"""Doctor evidence tiers, budgets, and shared-observation contracts."""
...
class DoctorEvidenceTierTests(unittest.TestCase):
    ...
    def test_windows_usn_token_rejects_zero_and_preserves_transform(self) -> None:
        self.assertIsNone(doctor.fileops._windows_usn_change_token(0))
        self.assertEqual(
            doctor.fileops._windows_usn_change_token(1),
            0x55534E5F46494CC5,
        )

    def test_windows_zero_usn_uses_content_change_token(self) -> None:
        path = Path("corpus.db")
        identity = (11, 22, 6, 33, 44)
        content_token = (1 << 256) | 9
        with (
            mock.patch.object(doctor.fileops, "WIN", True),
            mock.patch.object(
                doctor.fileops, "file_identity", return_value=identity),
            mock.patch.object(
                doctor.fileops, "windows_rust_change_token",
                return_value=None) as usn_token,
            mock.patch.object(
                doctor.fileops, "_content_change_token",
                return_value=content_token) as content,
        ):
            observed = doctor.fileops.change_sensitive_file_identity(path)
        self.assertEqual(observed, (*identity[:4], content_token))
        usn_token.assert_called_once_with(path, expected_identity=identity)
        content.assert_called_once_with(path, identity)

    @unittest.skipUnless(os.name == "nt", "Windows no-USN fallback")
    def test_windows_identity_hashes_when_usn_is_unavailable(self) -> None:
        with tempfile.TemporaryDirectory(
                prefix="agrep-doctor-content-token-") as td:
            path = Path(td) / "corpus.db"
            path.write_bytes(b"before")
            before_stat = path.stat()
            with (
    ...
    def test_timed_out_store_census_reaps_outside_the_budget(self) -> None:
        with tempfile.TemporaryDirectory(prefix="agrep-doctor-census-") as td:
            ...
            with (
                ...
                mock.patch.object(
                    indexd_runtime, "_new_drift_reaper",
                    return_value=reaper) as reaper_factory,
            ):
                self.assertIsNone(
        ...
        reaper_factory.assert_called_once_with(process)

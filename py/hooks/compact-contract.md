<!-- PreCompact hook payload. Appended to the summarizer's prompt as custom
     ...
     sections; do not renumber it behind the others.
...
10. Post-compact recovery: this summary is lossy, but the conversation it replaced is still indexed and partly recoverable. State under the heading "Post-compact recovery" as a gate, not a ritual: first name exactly what the next action needs and check the visible summary. If the summary fully states those facts, use it and do not retrieve history. If it only names or alludes to a needed exact value, decision, error text, path, failed approach, or unread result from before this boundary, the resumed agent's FIRST move is `agrep postcompact`, bounded rather than a dump, before acting or reconstructing from memory. Say explicitly that `agrep recall` is the wrong tool for this: when recall can identify the calling session it hides the current context window's echoes and demotes this session's older turns to ~self side-evidence (`--self` overrides), and it always ranks the actual tail against lookalikes from all history - while postcompact serves verbatim excerpts of the newest pre-boundary turns, marking what it omits, with no query to get right. If postcompact cannot identify the calling session, name it explicitly with `agrep postcompact --session <id>`. Acting on a guess where this summary is silent is the failure this section exists to prevent.

"""Goal 10 D3: freshness is drift, not duty-cycle.
...
class CallerOwnTranscript(unittest.TestCase):
    ...
    SESSION = "b0d9e7f0-4242-4000-8000-goal10caller"
...
class DriftProbeLifecycle(unittest.TestCase):
    def test_armed_probe_is_reaped_on_early_exit(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as raw:
            stub = Path(raw) / "ingest-stub"
            stub.write_text("#!/bin/sh\nsleep 30\n", encoding="ascii")
            stub.chmod(0o700)
            with mock.patch.object(common, "ingest_bin", return_value=stub):

"""Goal 10 audit evidence-book cache, disclosure, and exit-code contracts."""
...
class AuditBudgetTests(unittest.TestCase):
    ...
    def test_optional_and_sqlite_boundaries_use_native_change_time(self):
        with tempfile.TemporaryDirectory() as raw:
    ...
    @unittest.skipUnless(os.name == "nt", "native Windows ChangeTime contract")
    ...
    def test_windows_cache_snapshot_accepts_equivalent_device_encodings(self):
        with tempfile.TemporaryDirectory() as raw:
            ...
            with mock.patch.object(audit.os, "name", "nt"), \
                    mock.patch.object(
                        audit, "_cache_entry_owned", return_value=True), \
                    mock.patch.object(
                        audit.fileops, "file_identity",
    ...
    def test_readonly_data_dir_uses_scratch_cache_and_never_writes_protected(self):
        with tempfile.TemporaryDirectory() as raw:
            root = Path(raw)
            ...
            cached = json.loads(scratch_files[0].read_text(encoding="utf-8"))
    ...
    def test_combined_snapshot_degradation_is_rc2_evidence_error(self):
        issue = {
            "agent": "cursor",

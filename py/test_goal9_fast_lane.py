...
class Goal9FastLaneTests(unittest.TestCase):
    @staticmethod
    ...
    def _published_jsonl_fixture(self, root: Path) -> Path:
        ...
        signature = "goal9-fast-lane-publication"
    ...
    def test_allow_stale_serves_compatible_db_without_any_writer_primitive(
            self) -> None:
        with tempfile.TemporaryDirectory(prefix="agrep-goal9-fast-db-") as raw:
            root = Path(raw)
            messages = self._messages(root)
            ...
            # goal 10: the read lane records; only freshness_story renders
    ...
    def test_live_sqlite_contention_is_not_reported_as_foreign_owner(
        ...
        for dirty in (False, True):
            with self.subTest(dirty=dirty), tempfile.TemporaryDirectory(
                    prefix="agrep-goal9-live-lock-") as raw:
                root = Path(raw)
    ...
    def test_allow_stale_uses_direct_snapshot_when_fts_is_absent(
            self) -> None:
        with tempfile.TemporaryDirectory(prefix="agrep-goal9-fast-jsonl-") as raw:
            root = Path(raw)
            messages = self._messages(root)
            db_path = root / "corpus.db"
            stderr = io.StringIO()
    ...
    def test_count_spawns_daemon_and_scans_current_snapshot_without_rebuild(
            self) -> None:
        with tempfile.TemporaryDirectory(prefix="agrep-goal9-count-") as raw:
            root = Path(raw)
    ...
    def test_count_discloses_delegation_when_db_matches_published_messages(
        ...
        with tempfile.TemporaryDirectory(prefix="agrep-goal9-count-current-") as raw:
            root = Path(raw)
            messages = self._published_jsonl_fixture(root)
            db_path = root / "corpus.db"
    ...
    def test_source_publish_window_keeps_machine_search_on_last_good_db(
        ...
        with tempfile.TemporaryDirectory(
                prefix="agrep-goal9-source-publish-") as raw:
            root = Path(raw)
    ...
    def test_bounded_headline_count_stays_on_one_read_generation(self) -> None:
        ...
        with tempfile.TemporaryDirectory(
                prefix="agrep-goal9-query-snapshot-") as raw:
            root = Path(raw)
            db_path = root / "corpus.db"
            query = "publishneedle"
    ...
    def test_count_with_no_fts_scans_jsonl_and_leaves_db_absent(
            self) -> None:
        with tempfile.TemporaryDirectory(prefix="agrep-goal9-count-jsonl-") as raw:
            root = Path(raw)
            messages = self._published_jsonl_fixture(root)
            db_path = root / "corpus.db"
            stdout, stderr = io.StringIO(), io.StringIO()
    ...
    def test_count_without_trigram_discloses_delegated_direct_snapshot(
            self) -> None:
        with tempfile.TemporaryDirectory(prefix="agrep-goal9-count-no-trigram-") as raw:
            root = Path(raw)
            messages = self._published_jsonl_fixture(root)
            db_path = root / "corpus.db"
            stdout, stderr = io.StringIO(), io.StringIO()
        ...
        for readonly in (True, False):
            ...
            with (
                ...
                tempfile.TemporaryDirectory(
                    prefix=f"agrep-goal9-{policy}-surface-") as raw,
            ):
                root = Path(raw)
                self._published_fixture(root)
                db_path = root / "corpus.db"
                ...
                for surface_name, base_argv in surfaces.items():
                    ...
                    with self.subTest(
                        ...
                        # goal 10: never more than the one story line
    ...
    def test_semantic_ref_resolution_never_enters_the_fts_writer_lane(
            self) -> None:
        with tempfile.TemporaryDirectory(prefix="agrep-goal9-segment-") as raw:
            root = Path(raw)
            messages = self._messages(root)
            db_path = root / "corpus.db"
            self._database(db_path)

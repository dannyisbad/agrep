"""Goal 9 R6: pre-family publications have one truthful, terminating upgrade path."""
...
def _semantic_inputs(
    ...
    dim = 2
    generation = hashlib.md5(b"goal9-legacy-publication").digest()
    ...
    refs = [{
        ...
        "text_hash": text_hash,
        "agent": "claude",
        "project": "goal9-legacy-publication",
        "session": SESSION,
        "ts": 1,
        "turn": 0,
        "who": "user",
        "model": "claude-goal9-fixture",
...
class LegacyPublicationUpgradeTests(unittest.TestCase):
    ...
    def _fixture(root: Path) -> tuple[Path, Path, str]:
        home = root / "home"
        data = root / "data"
        project = home / ".claude" / "projects" / "goal9-legacy-publication"
    ...
    def test_v020_keyword_semantic_refusal_and_unchanged_ingest_upgrade(self) -> None:
        with tempfile.TemporaryDirectory(prefix="agrep-goal9-legacy-") as raw:
    ...
    def test_explicit_full_does_not_destroy_a_spent_adoption_clobber(
        ...
        with tempfile.TemporaryDirectory(
                prefix="agrep-goal9-post-adoption-clobber-") as raw:
    ...
    def test_stable_legacy_publication_does_not_pay_movement_retries(self) -> None:
        with tempfile.TemporaryDirectory(prefix="agrep-goal9-stable-legacy-") as raw:
            home, data, _term = self._fixture(Path(raw))
            first = self._ingest(home, data)
            self.assertEqual(first.returncode, 0, first.stderr)
            self._downgrade_to_v020(data)
    ...
    def test_family_meta_ahead_of_signature_is_a_publication_race(self) -> None:
        with tempfile.TemporaryDirectory(prefix="agrep-goal9-family-ahead-") as raw:
            data = Path(raw)
    ...
    def test_current_generation_missing_family_is_not_legacy(self) -> None:
        with tempfile.TemporaryDirectory(prefix="agrep-goal9-current-tear-") as raw:
            home, data, _term = self._fixture(Path(raw))
            first = self._ingest(home, data)
            self.assertEqual(first.returncode, 0, first.stderr)
            (data / session_context.SESSION_FAMILY_META_FILE).unlink()
    ...
    def test_malformed_proof_with_missing_family_fails_closed_as_unstable(self) -> None:
        with tempfile.TemporaryDirectory(prefix="agrep-goal9-malformed-proof-") as raw:
            home, data, _term = self._fixture(Path(raw))
            first = self._ingest(home, data)
            self.assertEqual(first.returncode, 0, first.stderr)
            (data / session_context.SESSION_FAMILY_META_FILE).unlink()
    ...
    def test_a_source_that_moves_mid_read_remains_unstable(self) -> None:
        with tempfile.TemporaryDirectory(prefix="agrep-goal9-moving-") as raw:
            home, data, _term = self._fixture(Path(raw))
            first = self._ingest(home, data)
            self.assertEqual(first.returncode, 0, first.stderr)
            real_snapshot = session_context.ownerfile.snapshot

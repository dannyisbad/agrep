...
enum Cmd {
    ...
    Scan {
        /// Which agent to ingest. (only `claude` wired so far)
...
fn main() -> anyhow::Result<()> {
    let cli = Cli::parse();
    match cli.cmd {
        Cmd::Scan { agent, min_msgs } => scan(&agent, min_msgs),
    }
}

fn scan(agent: &str, min_msgs: u64) -> anyhow::Result<()> {
    let t0 = Instant::now();
    let msgs = match agent {
        "claude" => ingest::claude::collect(),
        other => {
            anyhow::bail!("agent `{other}` not wired yet (have: claude)");
        }
    };
    ...
    println!("\n  tilt · {agent}");
    println!("  {} messages · {} vulgarity pts · {:.0} cumulative rage", n, tot_vulg, tot_rage);
    println!(
    ...
    for (i, l) in labels.iter().enumerate() {
        let pct = if n > 0 { tags[i] as f64 / n as f64 * 100.0 } else { 0.0 };
        print!("{l} {:.1}%  ", pct);
    }
    println!();
    ...
    for (p, a) in pv.iter().take(12) {
        println!("  {:<26} {:>6} {:>6} {:>6.2}", trunc(p, 26), a.msgs, a.vulgarity, a.vulgarity as f64 / a.msgs as f64);
    }
    ...
    pr.sort_by(|a, b| (b.1.rage / b.1.msgs as f64).partial_cmp(&(a.1.rage / a.1.msgs as f64)).unwrap());
    ...
    for (p, a) in pr.iter().take(12) {
        ...
        println!("  {:<26} {:>6} {:>9.2} {:>7.1}", trunc(p, 26), a.msgs, mean, fpct);
    }
    println!();
    Ok(())
}

//! Goal 8 real-process controls for Cursor token discovery and publication refusal.
...
fn cursor_schema_absent_is_a_successful_empty_publication() {
    let home = temp_dir("goal8-cursor-empty-home");
    let data = temp_dir("goal8-cursor-empty-data");
...
fn cursor_garbage_database_is_unreadable_and_aborts_publication() {
    let home = temp_dir("goal8-cursor-garbage-home");
    let data = temp_dir("goal8-cursor-garbage-data");
    let database = cursor_db(&home);
    fs::create_dir_all(database.parent().unwrap()).unwrap();
    fs::write(&database, b"not a sqlite database\0goal8").unwrap();
